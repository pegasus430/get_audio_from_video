...
def name(task):
    try:
        ...
        if "alt" in task.name:
            ...
            return """<div title="%s">%s</div>""" % (escape(task.name),
                                                        escape(short_name))
    except: return task.name

@display_field(_('filename'), 'filename')
def filename(task):
    try:
        data = task.args
        data = "args = " + data
        exec(data)
        _filename = args[0].get("filename")
        return _filename[:20]
    except: return ""

@display_field(_('duration'), 'duration')
def duration(task):
    try:
        data = task.args
        data = "args = " + data
        exec(data)
        _duration = args[0].get("duration")
        mins = _duration / 60
        seconds = _duration - mins * 60;
        return "%d:%d" % (mins, seconds)
    except: return ""

@display_field(_('format'), 'format')
def format(task):
    try:
        data = task.args
        data = "args = " + data
        exec(data)
        _format = args[0].get("format")
        return _format
    except: return ""

@display_field(_('username'), 'username')
def username(task):
    try:
        data = task.args
        data = "args = " + data
        exec(data)
        _username = args[0].get("username")
        return _username
    except:
        return ""
